...
export function AuthForm() {
  ...
  async function handleSubmit(event) {
    ...
    const { error } = await action(data.get('email'), data.get('password'));
  ...
  return <main className="auth-page">
    <section className="auth-introduction" aria-labelledby="auth-title">
      ...
      <p>Compare every acceptance criterion with a pull-request diff, and keep the evidence close to the verdict.</p>
    </section>
    <section className="auth-card" aria-labelledby="auth-form-title">
      <h2 id="auth-form-title">{mode === 'signIn' ? 'Welcome back' : 'Create your account'}</h2>
      ...
      <p>{mode === 'signIn' ? 'New to gaptrace?' : 'Already have an account?'} <button className="text-button" type="button" onClick={() => setMode(mode === 'signIn' ? 'signUp' : 'signIn')}>{mode === 'signIn' ? 'Create an account' : 'Sign in'}</button></p>
    </section>
  </main>;
}
